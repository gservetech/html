...
export function getNextContent() {
  ...
  if (files.length === 0) {
    logger.warn('Content', `No Instagram content files in queue! Add files to: ${config.contentQueueDir}`);
    return null;
  }

  logger.info('Content', `${files.length} Instagram file(s) in queue. Next: ${files[0].name}`);
  return files[0];
}
...
export function markAsPosted(filePath, results) {
  ...
  logger.info('Content', `Moved to Instagram posted: ${newName}`);
...
export function showQueueStatus() {
  ...
  console.log(`\n${'═'.repeat(60)}`);
  console.log('  📋  INSTAGRAM CONTENT QUEUE STATUS');
