...
export function getQueuedFiles() {
  return fs.readdirSync(config.contentQueueDir)
    .filter((fileName) => config.content.supportedFormats.includes(path.extname(fileName).toLowerCase()))
    .map((fileName) => {
...
export function getNextContent() {
  ...
  if (files.length === 0) {
    logger.warn('FB-Group-Content', `No Facebook group content files in queue. Add files to: ${config.contentQueueDir}`);
    return null;
  }

  logger.info('FB-Group-Content', `${files.length} file(s) in queue. Next: ${files[0].name}`);
  return files[0];
}
...
export function parseNextContent() {
  ...
  try {
    ...
    logger.error('FB-Group-Content', `Failed to parse ${file.name}: ${error.message}`);
    return null;
  }
}
...
export function markAsPosted(filePath, results) {
  ...
  logger.info('FB-Group-Content', `Moved to posted: ${newName}`);
...
export function showQueueStatus() {
  ...
  console.log(`\n${'═'.repeat(60)}`);
  console.log('  📋  FACEBOOK GROUP QUEUE STATUS');
