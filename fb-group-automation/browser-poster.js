...
async function loadPlaywright() {
  try {
    ...
    throw new Error(
      'This feature requires Playwright. Run "npm install" and then "npx playwright install chromium".',
    );
  }
}
...
async function openBrowser(headless = config.facebookGroup.headless) {
  ...
  const context = await chromium.launchPersistentContext(
    config.facebookGroup.browserProfileDir,
    {
      headless,
...
async function ensureLoggedIn(page) {
  ...
  if (
    ...
    throw new Error(
      'Facebook login is required. Run "npm run fbg:login" first and save the browser session.',
    );
  }

  // Attempt to close any pesky popups like "Create a PIN" or "Turn on notifications"
  try {
    const closeButtons = [
      page.getByRole("button", { name: "Close" }),
      page.getByLabel("Close"),
      page.locator('div[aria-label="Close"]'),
    ];
...
async function openComposer(page) {
  const opener = await findLocator(
    [
      page.getByRole("button", {
        name: /write something|create public post|what's on your mind/i,
      }),
      page
        .locator('[role="button"]')
        .filter({
          hasText: /write something|create public post|what's on your mind/i,
        }),
      page
  ...
  if (!opener) {
    throw new Error(
      "Could not find the group post composer. Open the group manually in the saved browser profile and confirm you can post there.",
    );
  }

  // Actually click the opener button
  await opener.click({ force: true }).catch(() => opener.dispatchEvent("click"));

  // Give the composer time to animate
  await page.waitForTimeout(2000);

  const editor = await findLocator(
    [
      page.locator('[role="dialog"] [data-lexical-editor="true"]'),
  ...
  if (!editor) {
    throw new Error(
      "The Facebook composer opened, but the message editor could not be found.",
    );
  }
...
async function ensurePostingIdentity(page) {
  ...
  if (!actorName) {
    logger.warn(
      "FB-Group",
      "FACEBOOK_GROUP_POST_AS_NAME is not set. The script cannot verify that Facebook will post as your Page.",
    );
    return;
  }
  ...
  const switcher = await findLocator(
    [
      ...
      page
        ...
        .filter({ hasText: /post as|posting as|profile|switch/i }),
    ],
    { timeout: 5000 },
  );
  ...
  if (!switcher) {
    throw new Error(
      `Could not find a posting-identity switcher for "${actorName}". Open "npm run fbg:login", switch to your Page manually, and save that session again.`,
    );
  }
  ...
  const actorOption = await findLocator(
    [
      ...
      page
        .locator('[role="dialog"] [role="button"]')
        .filter({ hasText: actorPattern }),
    ],
    { timeout: 8000 },
  );

  if (!actorOption) {
    await page.screenshot({ path: path.join(config.rootDir, 'diag-fail-actor.png') }).catch(() => {});
    await fs.writeFile(path.join(config.rootDir, 'diag-fail-actor.html'), await page.content()).catch(() => {});
  ...
  if (!confirmedActor) {
    throw new Error(
      `Facebook did not confirm the posting identity as "${actorName}". Stop and re-save the session with "npm run fbg:login".`,
    );
  }
...
async function writeMessage(page, editor, message) {
  ...
  if (
    await editor
      .evaluate((element) => element.tagName.toLowerCase() === "textarea")
      .catch(() => false)
  ) {
...
async function attachImage(page, parsedContent) {
  ...
  try {
    const photoButton = await findLocator(
      [
        page.getByRole("button", {
          name: /photo\/video|add photos|add photos\/videos/i,
        }),
        page
          .locator('[role="button"]')
          .filter({ hasText: /photo\/video|add photos|add photos\/videos/i }),
      ],
      { timeout: 4000 },
    );
...
async function submitPost(page) {
  ...
  if (!(await postButton.isEnabled().catch(() => false))) {
    throw new Error(
      "The post button never became enabled. The group may require extra prompts or moderation settings.",
    );
  }
...
export async function startGroupLoginSession() {
  if (!config.facebookGroup.enabled) {
    throw new Error(
      "Set FACEBOOK_GROUP_URL before starting the Facebook group login flow.",
    );
  }
  ...
  try {
    ...
    console.log("");
    console.log(
      `Step 2: The script opened your target group: ${config.facebookGroup.url}`,
    );
    console.log("Confirm that you can create a post in this group.");
    if (config.facebookGroup.postAsName) {
      console.log(
        `If needed, switch the posting identity to "${config.facebookGroup.postAsName}" before saving the session.`,
      );
    } else {
      console.log(
        "If you need to post as a Page, switch to that Page identity before saving the session.",
      );
    }
    console.log(
      "Leave the browser on the group page when everything looks correct.",
    );
    console.log("");
    ...
    logger.success(
      "FB-Group",
      `Saved browser session to ${config.facebookGroup.browserProfileDir}`,
    );
  } finally {
    rl.close();
    await context.close();
