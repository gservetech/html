...
export async function postToFacebook(parsedContent) {
  logger.info('Facebook', 'Preparing Facebook post...');
  ...
  try {
    ...
    logger.info('Facebook', `Post content (${message.length} chars)`);
    logger.debug('Facebook', message);
    ...
    logger.error('Facebook', `Failed to post: ${errorMsg}`);

    if (error.response?.data?.error?.code === 190) {
      logger.error('Facebook', 'Access token expired! Generate a new one at:');
      logger.error('Facebook', 'https://developers.facebook.com/tools/explorer/');
    }
...
export async function verifyFacebook() {
  try {
    ...
    logger.success('Facebook', `Connected to Page: "${response.data.name}"`);
    ...
    const errorMsg = error.response?.data?.error?.message || error.message;
    logger.error('Facebook', `Verification failed: ${errorMsg}`);
