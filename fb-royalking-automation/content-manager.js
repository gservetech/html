...
const LOG_SCOPE = 'Content-RoyalKing';
...
export function getNextContent() {
  ...
  if (files.length === 0) {
    logger.warn(LOG_SCOPE, `No Facebook content files in queue! Add files to: ${config.contentQueueDir}`);
    return null;
  }

  logger.info(LOG_SCOPE, `${files.length} Facebook file(s) in queue. Next: ${files[0].name}`);
  return files[0];
}
...
export function parseNextContent() {
  ...
  try {
    ...
    logger.error(LOG_SCOPE, `Failed to parse ${file.name}: ${error.message}`);
    return null;
  }
}
...
export function markAsPosted(filePath, results) {
  ...
  logger.info(LOG_SCOPE, `Moved to Facebook posted: ${newName}`);
...
export function showQueueStatus() {
  ...
  console.log(`\n${'═'.repeat(60)}`);
  console.log('  📋  ROYALKING FACEBOOK CONTENT QUEUE STATUS');
